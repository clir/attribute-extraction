 ...
 */
package edu.emory.clir.clearnlp.extraction.attribute.utils.chunk;
...
import edu.emory.clir.clearnlp.extraction.attribute.utils.TreeUtils;
...
public class Chunk extends Triple<String, DEPNode, List<DEPNode>> implements Comparable<Chunk>, Iterable<DEPNode>{
	...
	public Chunk(String o1, DEPNode o2, List<DEPNode> o3) {
		...
		l_strippedChunkNodes = TreeUtils.stripSubTree(new ArrayList<>(o3));
	...
	public IntIntPair getChunkSpan(){
		if(l_strippedChunkNodes.isEmpty())
			return new IntIntPair(getHeadNode().getID(), getHeadNode().getID());
		
		DEPNode first = l_strippedChunkNodes.get(0),
				last = l_strippedChunkNodes.get(l_strippedChunkNodes.size()-1);
		return new IntIntPair(first.getID(), last.getID());
	}
